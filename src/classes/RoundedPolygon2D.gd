...
func _build_rounded_uv() -> PackedVector2Array:
	...
	for i in range(polygon.size()):
		...
		for j in range(corner_detail + 1):
			...
			var polygon_bary = RoundedPolygon2D.get_2d_triangle_barycentric_coords(
				corner_point,
				polygon_triangle[0],
				polygon_triangle[1],
				polygon_triangle[2],
				)
			var uv_point = RoundedPolygon2D.barycentric_coords_to_cartesian(uv_triangle, polygon_bary)
...
func _build_vertex_colors() -> PackedColorArray:
	...
	for i in range(polygon.size()):
		...
		for j in range(corner_detail + 1):
			var corner_point = rounded_polygon[(i * (corner_detail + 1) + j) % rounded_polygon.size()]
			var bary = RoundedPolygon2D.get_2d_triangle_barycentric_coords(
				corner_point,
				polygon_triangle[0],
				polygon_triangle[1],
				polygon_triangle[2],
			...
			colors.append(RoundedPolygon2D.mix_colors(mix_colors, mix_weights))
	return colors
